...
async def get_image_news():
    """  """
    data_imag = script_params.get('data_imag', '')
    if data_imag == 'auto':
        data_imag = get_data_img()

    # 制作新闻的参数
    news_params = {
        'title': {
            'title1': script_params.get('title1') or 'AI小报亭',
            'title2': script_params.get('title2') or 'AI·大模型',
            'title3': script_params.get('title3') or '每日资讯',
            'title4': script_params.get('title4') or 'BREAKING NEWS',
            'title5': script_params.get('title5') or 'AI·LLM'
        },
        ...
        'sentence': script_params.get('sentence', -1),
    }
