...
from mediamate.tools.chart_video import LineTrend
...
def download_data() -> pd.DataFrame:
    ...
    gdp_data = round(gdp_data / 1e8, 4)
    gdp_data.index = pd.to_datetime(gdp_data.index, format='%Y')
    return gdp_data


async def get_xhs_line_trend(reset: bool = False):
    """  """
    ...
    for xhs in media_config.get('xhs', []):
        ...
        line_trend_path = os.path.join(media_path.upload, 'line_trend')
        if reset:
            shutil.rmtree(line_trend_path, ignore_errors=True)
        os.makedirs(line_trend_path, exist_ok=True)
        filename = 'gdp_data.csv'
        gdp_data.to_csv(f'{line_trend_path}/{filename}')
        title = '过去70年(GDP: 亿美元)'
        metadata = {
            '标题': title,
            '描述': '各国GPD历史变动',
        ...
        metadata_config = ConfigManager(f'{line_trend_path}/metadata.yaml')
        ...
        chart = LineTrend(line_trend_path)
        chart.create_video()
        logger.info(f'数据已保存至: {line_trend_path}')
        break


async def get_dy_line_trend(reset: bool = False):
    """  """
    gdp_data = download_data()
    media_config = config.MEDIA.get('media', {})
...
if __name__ == '__main__':
    ...
    # print(result)
    asyncio.run(get_xhs_line_trend(reset=True))
    asyncio.run(get_dy_line_trend(reset=True))
