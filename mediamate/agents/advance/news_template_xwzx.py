...
class NewsTemplateXwzx:
    ...
    async def get_html(self,
        ...
        template = env.get_template('news_xwzx.html')
    ...
    async def save_to_xhs(self, html_content: str, metadata: dict):
        ...
        for xhs in xhs_config:
            ...
            news_xwzx_path = os.path.join(media_path.upload, 'news_xwzx')
            if os.path.exists(news_xwzx_path):
                shutil.rmtree(news_xwzx_path)
            os.makedirs(news_xwzx_path, exist_ok=True)

            await self.save_html_content(html_content, news_xwzx_path)
            metadata_config = ConfigManager(f'{news_xwzx_path}/metadata.yaml')
            ...
            logger.info(f'数据已保存至: {news_xwzx_path}')
