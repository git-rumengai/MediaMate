...
class NewsTemplateZxzx:
    ...
    async def get_html(self,
        ...
        template = env.get_template('news_zxzx.html')
    ...
    async def save_to_xhs(self, html_content: str, metadata: dict):
        ...
        for xhs in xhs_config:
            ...
            news_zxzx_path = os.path.join(media_path.upload, 'news_zxzx')
            if os.path.exists(news_zxzx_path):
                shutil.rmtree(news_zxzx_path)
            os.makedirs(news_zxzx_path, exist_ok=True)

            await self.save_html_content(html_content, news_zxzx_path)
            metadata_config = ConfigManager(f'{news_zxzx_path}/metadata.yaml')
            ...
            logger.info(f'数据已保存至: {news_zxzx_path}')
