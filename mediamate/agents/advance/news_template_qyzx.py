...
class NewsTemplateQyzx:
    ...
    async def get_html(self,
        ...
        template = env.get_template('news_qyzx.html')
    ...
    async def save_to_xhs(self, html_content: str, metadata: dict):
        ...
        for xhs in xhs_config:
            ...
            news_qyzx_path = os.path.join(media_path.upload, 'news_qyzx')
            if os.path.exists(news_qyzx_path):
                shutil.rmtree(news_qyzx_path)
            os.makedirs(news_qyzx_path, exist_ok=True)

            await self.save_html_content(html_content, news_qyzx_path)
            metadata_config = ConfigManager(f'{news_qyzx_path}/metadata.yaml')
            ...
            logger.info(f'数据已保存至: {news_qyzx_path}')
