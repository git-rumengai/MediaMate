...
class NewsTemplateWddy:
    ...
    async def get_html(self,
        ...
        template_data['current_time'] = datetime.now().time().strftime('%H:%M')
        ...
        template = env.get_template('news_wddy.html')
    ...
    async def save_to_xhs(self, html_content: str, metadata: dict):
        ...
        for xhs in xhs_config:
            ...
            news_wddy_path = os.path.join(media_path.upload, 'news_wddy')
            if os.path.exists(news_wddy_path):
                shutil.rmtree(news_wddy_path)
            os.makedirs(news_wddy_path, exist_ok=True)

            await self.save_html_content(html_content, news_wddy_path)
            metadata_config = ConfigManager(f'{news_wddy_path}/metadata.yaml')
            ...
            logger.info(f'数据已保存至: {news_wddy_path}')
