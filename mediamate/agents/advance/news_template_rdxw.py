...
class NewsTemplateRdxw:
    ...
    async def get_html(self,
        ...
        template_data['subtitle'] = subtitle
        template_data['username'] = username
        template_data['date'] = datetime.today().date().strftime('%Y年%m月%d日')
        template_data['qr_code'] = qr_code
        news = await self.web_news.get_ddgs_news(keywords, blacklist=blacklist, limit=limit, days=days)
        news = [{'date': datetime.fromisoformat(i['date']).date().isoformat(), 'title': i['title'], 'body': i['body']} for i in news]
        template_data['news_list'] = news
        # 设置模板环境
        env = Environment(loader=FileSystemLoader(f'{config.PROJECT_DIR}/static/html'))
        template = env.get_template('news_rdxw.html')
    ...
    async def save_to_xhs(self, html_content: str, metadata: dict):
        ...
        for xhs in xhs_config:
            ...
            news_rdxw_path = os.path.join(media_path.upload, 'news_rdxw')
            if os.path.exists(news_rdxw_path):
                shutil.rmtree(news_rdxw_path)
            os.makedirs(news_rdxw_path, exist_ok=True)

            await self.save_html_content(html_content, news_rdxw_path)
            metadata_config = ConfigManager(f'{news_rdxw_path}/metadata.yaml')
            ...
            logger.info(f'数据已保存至: {news_rdxw_path}')
